...
const MainTextarea = ({
  ...
  const LIMIT_DESCRIPTION = `Limit reached! Your text exceeds ${limitValue.toString()} characters.`;
  return (
    <label
      className={`${isWarning ? 'h-[236px]' : 'h-[200px]'} relative mt-[48px] flex flex-col gap-[12px] [transition:300ms]`}
    >
      <textarea
        onInput={() => {
        ...
        ref={mainRef}
        id="main-text"
        className={`min-h-[200px] w-full resize-none rounded-[12px] border-2 bg-[#F2F2F7] p-[20px] text-[20px] leading-[140%] tracking-[-0.6px] text-[#2A2B37] outline-none [transition:background-color_300ms,color_300ms,border_300ms] dark:bg-[#2A2B37] dark:text-[#E4E4EF] ${isWarning ? 'border-[#DA3701] drop-shadow-[0_0_4px_rgba(218,55,1,0.75)] dark:border-[#FE8159] dark:drop-shadow-[0_0_4px_#FE8159]' : 'border-[#E4E4EF] dark:border-[#404254]'}`}
